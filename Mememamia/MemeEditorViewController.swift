...
class MemeEditorViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if (editMode) {
            setText(editModeMeme.topText!, bottomText: editModeMeme.bottomText!)
            setImage(editModeMeme.image!)
            topInput.clearsOnBeginEditing = false
            bottomInput.clearsOnBeginEditing = false
        }
    }
    ...
    @IBAction func shareButtonEvent(sender: UIBarButtonItem) {
        if let image = imageView.image {
            ...
            nextController.completionWithItemsHandler = { activity, success, items, error in
                if success {
                    self.saveMeme(image, memedImage: memedImage)
    ...
    private func cancelAndBackToInitialView() {
        imageView.image = nil
        initTextFields()
        disableElements([sharingButton])
    ...
    private func saveMeme(image: UIImage, memedImage: UIImage) {
        let meme = Meme(topText: self.topInput.text!, bottomText: self.bottomInput.text!, image: image, memedImage: memedImage, bounds: imageView.bounds)
    ...
    func setText(topText: String, bottomText: String) {
        topInput?.text = topText
        bottomInput?.text = bottomText
    }
    ...
    func resetTextConfig() {
        topInput.defaultTextAttributes = defaultTextFieldConfig
        topInput.textAlignment = NSTextAlignment.Center
        bottomInput.defaultTextAttributes = defaultTextFieldConfig
        bottomInput.textAlignment = NSTextAlignment.Center
    }
